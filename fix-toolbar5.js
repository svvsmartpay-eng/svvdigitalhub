...
let startIdx = lines.findIndex(l => l.includes('{/* Editing Tools Controls */}'));

if (startIdx !== -1) {
  let endIdx = startIdx;
  while (!lines[endIdx].includes('{/* Interactive Canvas Viewport */}')) {
    endIdx++;
  }
  
  let newToolbar = `{/* Editing Tools Controls */}
...
`;
  lines.splice(startIdx, endIdx - startIdx, newToolbar);
  fs.writeFileSync(path, lines.join('\n'));
  console.log('Fixed toolbar via splice');
} else {
