...
const endTagSearch = 'Crop Target Layers (Front/Back toggle or PDF)';

let startIndex = content.indexOf(startTag);
if (startIndex !== -1) {
  let endIndex = content.indexOf(endTagSearch, startIndex);
  if (endIndex !== -1) {
    // walk backwards to find the {
    let actualEndIndex = content.lastIndexOf('{/*', endIndex);
    
    let before = content.substring(0, startIndex);
    let after = content.substring(actualEndIndex); 
