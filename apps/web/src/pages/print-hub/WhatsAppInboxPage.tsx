...
export default function WhatsAppInboxPage() {
  ...
  const queueItems = React.useMemo(() => {
    // 1. Direct 1-to-1 Mapping of All Real Orders from Backend DB
    const ordersList = (ordersResponse?.data || []).map((ord: any) => {
      const ordDate = new Date(ord.createdAt);
      // Find linked WhatsApp media messages matching this customer's phone
      const matchingMessages = (messages || []).filter((m: any) => m.phone === ord.customerPhone);
      let mediaMessages = matchingMessages.filter((m: any) => m.mediaUrl);
      
      // If no media messages directly from chat, construct from order documentUrl
      if (mediaMessages.length === 0 && ord.documentUrl) {
        mediaMessages = [{
          id: `ord-doc-${ord.id}`,
          phone: ord.customerPhone,
          mediaUrl: ord.documentUrl,
          mediaType: ord.documentUrl.toLowerCase().endsWith('.pdf') ? 'PDF' : 'IMAGE',
          messageBody: ord.documentName || 'Document',
      ...
      let cleanFileName = ord.documentName || 'Document';
      ...
      return {
        ...
        latestMsg: matchingMessages[matchingMessages.length - 1] || null,
        mediaMessages,
        status,
    ...
    const orderPhones = new Set(ordersList.map((o: any) => o.phone));
    const unassignedChats: any[] = [];
    const chatMap: Record<string, boolean> = {};

    (messages || []).forEach((m: any) => {
      if (!orderPhones.has(m.phone) && !chatMap[m.phone] && m.mediaUrl) {
        chatMap[m.phone] = true;
        const msgDate = new Date(m.createdAt);
        const userMsgs = (messages || []).filter((msg: any) => msg.phone === m.phone);
        const mediaMsgs = userMsgs.filter((msg: any) => msg.mediaUrl);
        let custName = m.senderName || '';
        if (custName.includes('Print Desk') || custName.includes('SVV Communication')) custName = '';
        ...
        unassignedChats.push({
          id: `chat-${m.phone}`,
          phone: m.phone,
          customerName: custName || 'WhatsApp Customer',
          branch: m.branch,
          branchName: m.branch?.name || 'Isnapur',
          order: null,
          orderNo: '',
          tokenNumber: `WA-${m.phone.slice(-4)}`,
          latestMsg: m,
