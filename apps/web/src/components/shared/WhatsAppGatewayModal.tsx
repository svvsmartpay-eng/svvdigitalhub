...
import { useWhatsAppGatewayStatus, useStartWhatsAppGateway } from '@/api/printHub.api';
>>>>>>> Stashed changes
...
=======
  ...
  const { data: gatewayStatus, refetch: refetchGateway } = useWhatsAppGatewayStatus(branchId, open && sessionStatus === 'DISCONNECTED');
  const startGatewayMutation = useStartWhatsAppGateway();

  // ---------------------------------------------------------------------------
  // Generate a fresh WhatsApp Web-format QR (for pairing scan)
  // The QR encodes the raw session QR generated by Baileys on the backend.
  // ---------------------------------------------------------------------------
  const generateQR = useCallback(async () => {
    ...
    try {
      ...
      setQrCountdown(40); // Standard WhatsApp QR expiry is ~40s
    } finally {
      setLoading(false);
    }
  }, [branchId, startGatewayMutation, refetchGateway]);
  ...
  // Update rawQr from the live WebSocket session gateway payload
  useEffect(() => {
    if (gatewayStatus?.rawQr) {
      setRawQr(gatewayStatus.rawQr);
    } else if (gatewayStatus?.qrCodeDataUrl) {
      setRawQr(gatewayStatus.qrCodeDataUrl);
    }
  }, [gatewayStatus]);
...
=======
                        <div className="w-[300px] h-[300px] flex items-center justify-center text-gray-400 text-xs text-center p-4">
