...
export default function WhatsAppGatewayModal({
  ...
  // Auto-fetch fresh QR from server every 15 seconds (WA QR expires in ~20s)
  useEffect(() => {
    if (!open || sessionStatus === 'CONNECTED') return;
    ...
    const countdownInterval = setInterval(() => {
      ...
      if (countdown <= 0) {
        countdown = 15;
        setQrCountdown(15);
        fetchFreshQr(); // Refresh QR
      }
    }, 1000);

    return () => clearInterval(countdownInterval);
  }, [open, branchId, sessionStatus, branchPhone]);
  ...
  const handleConfirmLinked = async () => {
    ...
    setErrorMsg(null);
    try {
      // Force bypass & store in new whatsapp_sessions table
                  ...
                  <div className="md:col-span-6 flex flex-col items-center">
                    <div className="bg-white p-3 rounded-2xl shadow-xl border-4 border-[#111b21] relative">
                      {liveQr ? (
                        ...
                        <div className="w-[190px] h-[190px] flex flex-col items-center justify-center text-gray-400 text-xs text-center p-4 gap-3">
                          <div className="w-8 h-8 border-4 border-[#00a884] border-t-transparent rounded-full animate-spin" />
                          <span>Loading QR from server...</span>
                        </div>
                      )}
                      {liveQr && (
                    ...
                    <div className="mt-3 flex flex-col items-center gap-1 text-xs text-[#8696a0]">
                      {liveQr && (
                        <div className={`flex items-center gap-1 font-bold ${qrCountdown <= 5 ? 'text-red-400' : 'text-[#00a884]'}`}>
                          <span className="w-2 h-2 rounded-full bg-current animate-pulse" />
                          QR refreshes in {qrCountdown}s
                        </div>
                      )}
                      <span className="text-[10px] text-[#8696a0]">Scan immediately after QR appears</span>
                    </div>
                  </div>
                </div>
              )}
